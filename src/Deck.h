...
public:
  ...
  bool operator==(const Deck &other) const {
    if (deck.size() != other.deck.size()) {
        return false; // Špilovi različite veličine ne mogu biti jednaki
    }

    // Pretvaranje vektora u skupove za poređenje
    std::set<Card> thisDeckSet(deck.begin(), deck.end());
    std::set<Card> otherDeckSet(other.deck.begin(), other.deck.end());

    return thisDeckSet == otherDeckSet;
}
private:
  vector<Card> deck;
  Card topCard;
